...
vi.mock("../api/pedidos.js", () => ({ listarPedidosGerais: vi.fn() }));
...
describe("Pedidos (fila)", () => {
  ...
  it("lista os pedidos do setor", async () => {
    listarPedidosGerais.mockResolvedValue(LISTA);
    renderApp(<Pedidos />);
  ...
  it("filtra por status ao clicar numa aba", async () => {
    ...
    // a última chamada deve incluir o status "pendente" (1º e único arg)
    const ultima = listarPedidosGerais.mock.calls.at(-1);
    expect(ultima[0]).toBe("pendente");
  });

  it("mostra estado vazio quando não há pedidos", async () => {
    listarPedidosGerais.mockResolvedValue([]);
    renderApp(<Pedidos />);
